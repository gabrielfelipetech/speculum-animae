...
import { buildTwelveLayersPdfContent } from './twelveLayersPdf.mjs';
import { c as collectPdfBuffer, a as createLayoutState, w as writeCover, d as writeH2, f as writeParagraph, b as writeDivider, g as writeBulletList, i as drawHeaderFooter, j as ensureSpace, h as writeCallout, e as writeH3 } from '../../../_/layout.mjs';
...
function writeScoreOverview(doc, state, content) {
  ...
  for (const layer of content.orderedScores) {
    ensureSpace(doc, 42);
    doc.font("Helvetica-Bold").fontSize(11).fillColor("#111827").text(layer.name, {
      continued: true
    }).font("Helvetica-Oblique").fontSize(10.5).fillColor("#6B7280").text(`  Media (0-10): ${layer.score.toFixed(2)}`, {
  ...
  writeCallout(
    doc,
    state,
    `Camadas de maior intensidade: ${content.topLayers.map((layer) => layer.name).join(", ")}. Camadas com maior necessidade de reforco: ${content.bottomLayers.map((layer) => layer.name).join(", ")}.`
  );
  writeDivider(doc, state);
}
function writeLayerSections(doc, state, content) {
...
function renderTwelveLayersPdf(doc, entry, content) {
  ...
  writeCover(
    ...
    content.subtitle,
    {
      ...
      summary: "Este PDF detalha cada camada com descricao, forcas, riscos e praticas, seguido de plano de melhoria em 7 e 30 dias."
    }
  );
...
async function generateTwelveLayersPdfBinary(entry) {
...
export { generateTwelveLayersPdfBinary };
//# sourceMappingURL=twelveLayersPdfRender.mjs.map
