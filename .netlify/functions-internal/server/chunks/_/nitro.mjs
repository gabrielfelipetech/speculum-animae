import process from 'node:process';globalThis._importMeta_=globalThis._importMeta_||{url:"file:///_entry.js",env:process.env};import Bugsnag from '@bugsnag/js';
...
const _inlineRuntimeConfig = {
  "app": {
    "baseURL": "/",
    "buildId": "e1f7aa17-2420-4fcb-900e-189239d6a520",
  ...
  "public": {
    "siteUrl": "https://speculumanimae.com.br",
    "stripePublicKey": "",
    "bugsnagApiKey": "",
    "appVersion": "dev",
    "releaseStage": "",
    "supabaseUrl": "https://otjbkxaalohbmgasouah.supabase.co",
    "supabaseAnonKey": "sb_publishable_O5oF4_TDmOxaI2TQu16lZw_ca7t_5pL",
    "supabase": {
      "url": "https://otjbkxaalohbmgasouah.supabase.co",
      "key": "sb_publishable_O5oF4_TDmOxaI2TQu16lZw_ca7t_5pL",
      "redirect": false,
      "redirectOptions": {
        "login": "/login",
        "callback": "/confirm",
      ...
      "cookieName": "sb",
      "cookiePrefix": "sb-otjbkxaalohbmgasouah-auth-token",
      "useSsrCookies": true,
      "cookieOptions": {
        "maxAge": 28800,
        "sameSite": "lax",
  ...
  "content": {
    ...
    "localDatabase": {
      "type": "sqlite",
      "filename": "C:/Users/gabri/Projetos/Pessoais/speculum-animae-nuxt/.data/content/contents.sqlite"
    },
    "integrityCheck": true
  },
  "nuxt-site-config": {
    "stack": [
      ...
      },
      {
        ...
        "url": "https://speculumanimae.com.br",
...
const _Tq7U_lLWNdAQao3AwevNX5wr3k8Z942jsXOKwgu1GJI = defineNitroPlugin(async (nitroApp) => {
  ...
  "pt-BR": [
    {
      ...
      load: () => import('./pt-BR.mjs' /* webpackChunkName: "locale_pt_45BR_46json_522067c3" */),
      cache: true
    }
  ],
  en: [
    {
      ...
      load: () => import('./en.mjs' /* webpackChunkName: "locale_en_46json_0de83a29" */),
      cache: true
    }
  ],
  es: [
    {
      ...
      load: () => import('./es.mjs' /* webpackChunkName: "locale_es_46json_28f00529" */),
      cache: true
    }
  ]
};
const vueI18nConfigs = [
  () => import('./i18n.config.mjs' /* webpackChunkName: "config_i18n_46config_46ts_bcce9c12" */)
];
...
async function resolveAuthUser(event) {
  ...
  const token = (_a = match[1]) == null ? void 0 : _a.trim();
  if (!token) return null;
  try {
    const supabase = await serverSupabaseClient(event);
    const { data, error } = await supabase.auth.getUser(token);
    if (error) return null;
    if (data.user && isValidUuid$1(data.user.id)) {
      return data.user;
    }
  } catch {
    return null;
...
function resolveReleaseStage(config) {
  var _a;
  const explicit = safeString((_a = config.public) == null ? void 0 : _a.releaseStage);
  if (explicit) return explicit;
  return "production" ;
}
function resolveAppVersion(config) {
  var _a, _b;
  return (_b = safeString((_a = config.public) == null ? void 0 : _a.appVersion)) != null ? _b : safeString(process.env.NUXT_PUBLIC_APP_VERSION);
}
...
function safeStartServerBugsnag() {
  var _a, _b;
  try {
    const config = useRuntimeConfig();
    const apiKey = (_b = safeString((_a = config.public) == null ? void 0 : _a.bugsnagApiKey)) != null ? _b : safeString(process.env.NUXT_PUBLIC_BUGSNAG_API_KEY);
...
function getServerBugsnag() {
  return safeStartServerBugsnag();
}
function leaveServerBreadcrumb(name, metadata) {
  try {
    const client = getServerBugsnag();
    if (!client) return;
    client.leaveBreadcrumb(name, metadata != null ? metadata : {});
  } catch {
  }
}
function notifyServerError(event, error, metadata) {
  var _a, _b;
  try {
    const client = getServerBugsnag();
    if (!client) return;
    ...
    const method = (_a = event.node.req.method) != null ? _a : "GET";
    const userAgent = (_b = getRequestHeader(event, "user-agent")) != null ? _b : void 0;
...
async function withCriticalApiLogging(event, ctx, fn) {
  var _a, _b, _c;
  ...
  leaveServerBreadcrumb("api.request", {
    ...
    method,
    path,
    authUserId: (_b = ctx.authUserId) != null ? _b : null
  });
  try {
    ...
    notifyServerError(event, error, {
      area: ctx.area,
      durationMs,
      authUserId: (_c = ctx.authUserId) != null ? _c : null,
      method,
      path
    });
    throw error;
...
async function resolveUserId(event) {
  ...
  try {
    const user = await serverSupabaseUser(event).catch(() => null);
    if (isValidUuid(user == null ? void 0 : user.id)) {
      return user.id;
    }
  } catch {
  }
  ...
  try {
    ...
    const { data, error } = await supabase.auth.getUser(token);
    if (error) return null;
    if (isValidUuid((_b = data.user) == null ? void 0 : _b.id)) {
      return data.user.id;
    }
  } catch {
    return null;
...
const appHead = {"meta":[{"charset":"utf-8"},{"name":"viewport","content":"width=device-width, initial-scale=1"},{"name":"description","content":"Plataforma de testes de personalidade, temperamento e virtudes."},{"name":"theme-color","content":"#0f172a"}],"link":[{"rel":"icon","href":"/favicon.ico"},{"rel":"icon","type":"image/png","sizes":"48x48","href":"/favicon-48.png"},{"rel":"apple-touch-icon","sizes":"180x180","href":"/apple-touch-icon.png"}],"style":[],"script":[],"noscript":[],"htmlAttrs":{"lang":"pt-BR"}};
...
const _AeFOvfM3Okg5YIah8uMRuOfIJHf1K88b05BYv207U = defineNitroPlugin(async (nitro) => {
  const runtimeI18n = useRuntimeI18n();
  ...
  nitro.hooks.hook("render:before", async ({ event }) => {
    ...
    if (!url.pathname.includes("/_i18n/mjoZES-1") && !isExistingNuxtRoute(path)) {
      return;
    }
...
const plugins = [
  _Tq7U_lLWNdAQao3AwevNX5wr3k8Z942jsXOKwgu1GJI,
_AeFOvfM3Okg5YIah8uMRuOfIJHf1K88b05BYv207U
];
...
const _hLBXmX = defineCachedEventHandler(async (event) => {
  const url = getRequestURL(event);
  if (!url)
...
const _eeRb_C = eventHandler(async (e) => {
...
const _a4bTt7 = defineEventHandler(async (e) => {
  const collections = [];
...
const _38dBcn = defineEventHandler(async (e) => {
...
const _dt9h5f = defineEventHandler(sitemapXmlEventHandler);
...
const _JGrkhJ = _messagesHandlerCached;

const _FDTWmU = eventHandler(async (event) => {
...
const _2Z7XUs = eventHandler(async (event) => {
...
const _lazy_TieWgz = () => import('../routes/api/billing/checkout.post.mjs');
const _lazy_Y9NrnC = () => import('../routes/api/billing/plans.get.mjs');
const _lazy_JWcmiW = () => import('../routes/api/billing/portal.post.mjs');
const _lazy_iNecjH = () => import('../routes/api/my-results.get.mjs');
const _lazy_r43M9Z = () => import('../routes/api/report-builders/assessments.mjs');
const _lazy_pjXq5Y = () => import('../routes/api/report-builders/dispatch.mjs');
const _lazy_OTEdvB = () => import('../routes/api/report-builders/temperaments.mjs');
const _lazy_rNVvUa = () => import('../routes/api/report-builders/temperamentsCompatibility.mjs');
const _lazy_YZnc8z = () => import('../routes/api/report-builders/temperamentsCompatibilityPdf.mjs');
const _lazy_133y73 = () => import('../routes/api/report-builders/temperamentsCompatibilityPdfRender.mjs');
const _lazy_Hq0Wkm = () => import('../routes/api/report-builders/temperamentsPdf.choleric.mjs');
const _lazy_6_YqfZ = () => import('../routes/api/report-builders/temperamentsPdf.melancholic.mjs');
const _lazy_klkP46 = () => import('../routes/api/report-builders/temperamentsPdf.phlegmatic.mjs');
const _lazy_qWosvb = () => import('../routes/api/report-builders/temperamentsPdf.sanguine.mjs');
const _lazy_8GNMRf = () => import('../routes/api/report-builders/temperamentsPdf.mjs');
const _lazy__jDD4v = () => import('../routes/api/report-builders/temperamentsPdfRender.mjs');
const _lazy_1n9mIQ = () => import('../routes/api/report-builders/twelveLayers.mjs');
const _lazy_eyCWDB = () => import('../routes/api/report-builders/twelveLayersPdf.mjs');
const _lazy_BFEbUx = () => import('../routes/api/report-builders/twelveLayersPdfRender.mjs');
const _lazy_aNAWYX = () => import('../routes/api/results.get.mjs');
const _lazy_cJ5k8w = () => import('../routes/api/results.post.mjs');
const _lazy_swD7q2 = () => import('../routes/api/results/_id_.get.mjs');
const _lazy_nEYQxn = () => import('../routes/api/results/_id/pdf.get.mjs');
const _lazy__X667X = () => import('../routes/renderer.mjs').then(function (n) { return n.r; });

const handlers = [
  { route: '/api/billing/checkout', handler: _lazy_TieWgz, lazy: true, middleware: false, method: "post" },
  { route: '/api/billing/plans', handler: _lazy_Y9NrnC, lazy: true, middleware: false, method: "get" },
  { route: '/api/billing/portal', handler: _lazy_JWcmiW, lazy: true, middleware: false, method: "post" },
  { route: '/api/my-results', handler: _lazy_iNecjH, lazy: true, middleware: false, method: "get" },
  { route: '/api/report-builders/assessments', handler: _lazy_r43M9Z, lazy: true, middleware: false, method: undefined },
  { route: '/api/report-builders/dispatch', handler: _lazy_pjXq5Y, lazy: true, middleware: false, method: undefined },
  { route: '/api/report-builders/temperaments', handler: _lazy_OTEdvB, lazy: true, middleware: false, method: undefined },
  { route: '/api/report-builders/temperamentsCompatibility', handler: _lazy_rNVvUa, lazy: true, middleware: false, method: undefined },
  { route: '/api/report-builders/temperamentsCompatibilityPdf', handler: _lazy_YZnc8z, lazy: true, middleware: false, method: undefined },
  { route: '/api/report-builders/temperamentsCompatibilityPdfRender', handler: _lazy_133y73, lazy: true, middleware: false, method: undefined },
  { route: '/api/report-builders/temperamentsPdf.choleric', handler: _lazy_Hq0Wkm, lazy: true, middleware: false, method: undefined },
  { route: '/api/report-builders/temperamentsPdf.melancholic', handler: _lazy_6_YqfZ, lazy: true, middleware: false, method: undefined },
  { route: '/api/report-builders/temperamentsPdf.phlegmatic', handler: _lazy_klkP46, lazy: true, middleware: false, method: undefined },
  { route: '/api/report-builders/temperamentsPdf.sanguine', handler: _lazy_qWosvb, lazy: true, middleware: false, method: undefined },
  { route: '/api/report-builders/temperamentsPdf', handler: _lazy_8GNMRf, lazy: true, middleware: false, method: undefined },
  { route: '/api/report-builders/temperamentsPdfRender', handler: _lazy__jDD4v, lazy: true, middleware: false, method: undefined },
  { route: '/api/report-builders/twelveLayers', handler: _lazy_1n9mIQ, lazy: true, middleware: false, method: undefined },
  { route: '/api/report-builders/twelveLayersPdf', handler: _lazy_eyCWDB, lazy: true, middleware: false, method: undefined },
  { route: '/api/report-builders/twelveLayersPdfRender', handler: _lazy_BFEbUx, lazy: true, middleware: false, method: undefined },
  { route: '/api/results', handler: _lazy_aNAWYX, lazy: true, middleware: false, method: "get" },
  { route: '/api/results', handler: _lazy_cJ5k8w, lazy: true, middleware: false, method: "post" },
  { route: '/api/results/:id', handler: _lazy_swD7q2, lazy: true, middleware: false, method: "get" },
  { route: '/api/results/:id/pdf', handler: _lazy_nEYQxn, lazy: true, middleware: false, method: "get" },
  { route: '/__nuxt_error', handler: _lazy__X667X, lazy: true, middleware: false, method: undefined },
  { route: '/__nuxt_island/**', handler: _SxA8c9, lazy: false, middleware: false, method: undefined },
  { route: '/api/_nuxt_icon/:collection', handler: _hLBXmX, lazy: false, middleware: false, method: undefined },
  { route: '', handler: _eeRb_C, lazy: false, middleware: true, method: undefined },
  { route: '/__sitemap__/nuxt-content-urls.json', handler: _a4bTt7, lazy: false, middleware: false, method: undefined },
  { route: '/__sitemap__/style.xsl', handler: _38dBcn, lazy: false, middleware: false, method: undefined },
  { route: '/sitemap.xml', handler: _dt9h5f, lazy: false, middleware: false, method: undefined },
  { route: '/_i18n/:hash/:locale/messages.json', handler: _JGrkhJ, lazy: false, middleware: false, method: undefined },
  { route: '/__nuxt_content/:collection/sql_dump.txt', handler: _FDTWmU, lazy: false, middleware: false, method: undefined },
  { route: '/__nuxt_content/:collection/query', handler: _2Z7XUs, lazy: false, middleware: false, method: undefined },
  { route: '/**', handler: _lazy__X667X, lazy: true, middleware: false, method: undefined }
];
...
export { $fetch$1 as $, defineRenderHandler as A, publicAssetsURL as B, appTeleportTag as C, appTeleportAttrs as D, appHead as E, destr as F, getRouteRules as G, parseQuery as H, hasProtocol as I, isScriptProtocol as J, joinURL as K, klona as L, defuFn as M, withQuery as N, sanitizeStatusCode as O, withTrailingSlash as P, withoutTrailingSlash as Q, getContext as R, baseURL as S, createHooks as T, executeAsync as U, parseURL as V, toRouteMatcher as W, createRouter$1 as X, defu as Y, setCookie as Z, getCookie as _, useRuntimeConfig as a, getRequestHeader as a0, createDefu as a1, isEqual as a2, parsePath as a3, deleteCookie as a4, withLeadingSlash as a5, getRequestURL as b, createError$1 as c, defineEventHandler as d, resolveAuthUser as e, getQuery as f, getRouteRulesForPath as g, getHeader as h, useStorage as i, joinHeaders as j, getRouterParam as k, resolveUserId as l, setHeader as m, normalizeCookieHeader as n, buildAssetsURL as o, appRootTag as p, appRootAttrs as q, readBody as r, serverSupabaseClient as s, appSpaLoaderTag as t, useNitroApp as u, appSpaLoaderAttrs as v, withCriticalApiLogging as w, getResponseStatusText as x, getResponseStatus as y, appId as z };
