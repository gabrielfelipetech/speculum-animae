...
import { TEMPERAMENT_FILENAME_SLUGS } from '../../texts/premiumTemperaments';
import { buildTemperamentsCompatibilityPdfContent } from './temperamentsCompatibilityPdf';
import {
  collectPdfBuffer,
  createLayoutState,
  drawHeaderFooter,
...
export type TemperamentsCompatibilityPdfBinary = {
  fileName: string;
  buffer: Buffer;
};
...
function writeScoreOverview(
  ...
  content: ReturnType<typeof buildTemperamentsCompatibilityPdfContent>,
): void {
  ...
  for (const score of content.scoresOrdered) {
    ...
    doc
      ...
      .fontSize(10.5)
      .fillColor('#6B7280')
      .text(`  Media (0-10): ${score.score.toFixed(2)}`, {
...
function renderCompatibilityPdf(
  doc: PDFKit.PDFDocument,
  entry: StoredResult,
  content: ReturnType<typeof buildTemperamentsCompatibilityPdfContent>,
): void {
  ...
  writeCover(
    ...
    'Leitura do par com foco em dinamicas, atritos e acordos praticos.',
    {
      ...
      summary:
        'Documento estruturado em escala 0-10 com analise do par, estrategias de reparacao e plano de acordos em ciclos de 7 e 30 dias.',
    },
  );
...
export async function generateTemperamentsCompatibilityPdfBinary(
  entry: StoredResult,
): Promise<TemperamentsCompatibilityPdfBinary> {
  ...
  const fileName =
    `relatorio-compatibilidade-temperamentos-${primarySlug}-${secondarySlug}.pdf`;
